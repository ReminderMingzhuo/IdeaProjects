...
public class CS570Test2 {
    public static void main(String[] args) {
        int[][] manPref = {{1,0,2},   //manPref[0][] for John, 0 is John's index
                {0,2,1},   //manPref[1][] for Jack, 1 is Jack's index
                {2,0,1}};   //manPref[2][] for Tom,  2 is Tom's index
        int[][] womanPref = {{0,1,2}, //womanPref[0][] for Luna, 0 is Luna's index
                {0,2,1}, //womanPref[1][] for Eva, 1 is Luna's index, {John, Tom, Jack} Eva like John most, and then Tom, Jack is the last
...
//Preparation
        ...
        for(int i = 0; i < womanPref.length ; i++) {
            for(int j = 0; j < womanPref[0].length; j++){
                womanPref2[i][womanPref[i][j]] = j; // i is the woman's index, woman[i][j] is man's index that she jth like, j is the rank of the man in woman i's mind
            }
        }
        ...
        //John proposes to Eva. Tom is her current mate. Who will stay engaging with Eva?
        int ppMan = 0; //John is the man that proposing on this turn of iteration
        ...
        System.out.println("After this interation, "+women[ppedWoman]+" engage "+men[curEvaMan]+" now.");
    }
